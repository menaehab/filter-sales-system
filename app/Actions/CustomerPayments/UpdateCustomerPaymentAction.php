...
final class UpdateCustomerPaymentAction
{
    public function execute(CustomerPayment $payment, array $data): CustomerPayment
    {
        return DB::transaction(function () use ($payment, $data): CustomerPayment {
            $payment->load('allocations.sale');
    ...
    private function syncAllocations(CustomerPayment $payment, float $oldTotal, float $newTotal): void
    {
    ...
    private function maxAllocatableAmount(CustomerPayment $payment): float
    {
        return round((float) $payment->allocations
            ->map(function ($allocation) use ($payment) {
                ...
                $otherPaid = (float) $allocation->sale->paymentAllocations()
                    ->where('customer_payment_id', '!=', $payment->id)
                    ->sum('amount');

                return max(0, (float) $allocation->sale->total_price - $otherPaid);
            })
            ->sum(), 2);
    }
}
