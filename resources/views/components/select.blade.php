...
<div {{ $attributes->only('class')->merge(['class' => 'w-full space-y-1.5']) }}>
    ...
    <select id="{{ $name }}" name="{{ $multiple ? $name . '[]' : $name }}" {{ $required ? 'required' : '' }}
        {{ $disabled ? 'disabled' : '' }} {{ $multiple ? 'multiple' : '' }}
        {{ $attributes->except('class')->merge([
            'class' =>
                'block w-full rounded-xl border bg-white py-2.5 ps-3 pe-9 text-sm shadow-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-0 disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-500' .
                ($errors->has($name) || $error
                    ? ' border-red-300 text-red-900 focus:border-red-500 focus:ring-red-500/70'
        ...
        @else
            @foreach ($options as $value => $optionLabel)
                <option value="{{ $value }}"
                    ...
                    @else
                        {{ (string) $selectedValue === (string) $value ? 'selected' : '' }} @endif>
                    {{ $optionLabel }}
                </option>
            @endforeach
        @endif
