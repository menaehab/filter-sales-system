<div x-on:confirmed-delete-supplier.window="$wire.delete()">
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-900">{{ __('keywords.suppliers') }}</h2>
            <p class="mt-1 text-sm text-gray-500">{{ __('keywords.suppliers_management') }}</p>
        </div>
    ...
    <x-data-table :searchable="false" :paginated="false" :headers="[
        ...
        @forelse ($this->suppliers as $supplier)
            <tr class="hover:bg-gray-50">
                ...
                <td class="whitespace-nowrap px-4 py-3">
                    <div class="flex items-center gap-3">
                        <span class="text-sm font-medium text-gray-900">{{ $supplier->phone }}</span>
                    </div>
                </td>
                <td class="whitespace-nowrap px-4 py-3 text-end text-sm">
                    <div class="flex items-center justify-end gap-2">
                        <a href="{{ route('suppliers.show', $supplier) }}"
                            class="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-blue-600" title="View">
                        ...
                        <button class="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-emerald-600"
                            title="Edit" wire:click="openEdit({{ $supplier->id }})">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <button class="rounded-lg p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-600" title="Delete"
                            wire:click="setDelete({{ $supplier->id }})">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round"
        ...
        @empty
            <tr>
                <td colspan="3" class="px-4 py-8 text-center text-sm text-gray-500">
                    {{ __('keywords.no_suppliers_found') }}</td>
            </tr>
        @endforelse
    </x-data-table>
    ...
    <div class="mt-4 flex flex-col items-center justify-between gap-4 sm:flex-row">
        <p class="text-sm text-gray-500">
            {{ __('keywords.showing') }} <span
                class="font-medium text-gray-700">{{ $this->suppliers->firstItem() ?? 0 }}</span>
            {{ __('keywords.to') }} <span
                class="font-medium text-gray-700">{{ $this->suppliers->lastItem() ?? 0 }}</span>
            {{ __('keywords.of') }} <span class="font-medium text-gray-700">{{ $this->suppliers->total() }}</span>
            {{ __('keywords.results') }}
        </p>
        <div>
