        ...
        <div class="xl:col-span-1 space-y-6">
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm p-5 space-y-3">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-medium text-gray-900">{{ $customer->phone ?? __('keywords.not_specified') }}</span>
                </div>
                <div class="flex justify-between text-sm">
            ...
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm p-5 space-y-3">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-bold text-gray-900">{{ number_format($customer->total_sales, 2) }} {{ __('keywords.currency') }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">{{ __('keywords.total_payments') }}</span>
                    <span class="font-medium text-emerald-600">{{ number_format($customer->total_payments, 2) }} {{ __('keywords.currency') }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">{{ __('keywords.total_returns_without_cash') }}</span>
                    <span class="font-medium text-amber-600">{{ number_format($customer->total_returns_without_cash, 2) }} {{ __('keywords.currency') }}</span>
                </div>
                <div class="flex justify-between text-sm border-t pt-3">
                    <span class="text-gray-700 font-medium">{{ __('keywords.balance') }}</span>
                    <span class="font-bold {{ $customer->balance > 0 ? 'text-red-600' : ($customer->balance < 0 ? 'text-emerald-600' : 'text-gray-500') }}">
            ...
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                <div class="flex border-b bg-white">
                    ...
                    <button wire:click="setActiveTab('returns')"
                        ...
                        {{ __('keywords.sale_returns') }} ({{ $customer->sales()->with('saleReturns')->get()->flatMap(fn($sale) => $sale->saleReturns()->where('cash_refund', false)->pluck('id'))->count() }})
                    </button>
                </div>
                ...
                @if($activeTab === 'sales')
                    <div class="overflow-x-auto">
                        @if($sales->isEmpty())
                            <div class="px-4 py-8 text-center text-sm text-gray-500">{{ __('keywords.no_sales_found') }}</div>
                        @else
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach($sales as $sale)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $sale->number }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-700 font-medium">{{ number_format($sale->total_price, 2) }} {{ __('keywords.currency') }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-600">{{ $sale->items->count() }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-500">{{ $sale->created_at->format('Y-m-d H:i') }}</td>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('keywords.sale') }}</th>
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach($payments as $i => $payment)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-4 py-3 text-sm text-gray-600">{{ $payment->payment_method === 'customer_credit' ? __('keywords.applied_customer_credit') : $payment->payment_method }}</td>
                ...
                @if($activeTab === 'returns')
                    <div class="overflow-x-auto">
                        @if($returns->isEmpty())
                            <div class="px-4 py-8 text-center text-sm text-gray-500">{{ __('keywords.no_sale_returns_found') }}</div>
                        @else
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('keywords.number') }}</th>
                                        <th class="px-4 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('keywords.sale_number') }}</th>
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach($returns as $return)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-4 py-3 text-sm text-gray-600">{{ $return->sale->number }}</td>
                                            ...
                                            <td class="px-4 py-3 text-end text-sm">
                                                <a href="{{ route('sale-returns.show', $return) }}" class="text-blue-600 hover:text-blue-900">
                                                    {{ __('keywords.view') }}
                                                </a>
                                            </td>
                                        </tr>
