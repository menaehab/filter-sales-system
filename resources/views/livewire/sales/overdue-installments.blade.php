<div>
    ...
    @if ($overdueSales->isNotEmpty())
        <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
            <x-stat-card
                :label="__('keywords.overdue_count')"
                :value="$overdueSales->total()"
                icon-class="fas fa-clock"
                color="red"
            />
            <x-stat-card
                :label="__('keywords.total_overdue_installments')"
                :value="number_format(collect($overdueSales->items())->sum('installment_amount'), 2)"
                :suffix="__('keywords.currency')"
                icon-class="fas fa-money-bill-wave"
                color="amber"
            />
            <x-stat-card
                :label="__('keywords.total_remaining_all')"
                :value="number_format(collect($overdueSales->items())->sum('remaining_amount'), 2)"
                :suffix="__('keywords.currency')"
    ...
    {{-- Search Toolbar --}}
    <x-search-toolbar
        :searchPlaceholder="__('keywords.search_by_customer_or_invoice')"
        searchModel="search"
    ...
    <x-data-table :searchable="false" :paginated="false" :headers="[
        ...
        @forelse ($overdueSales as $sale)
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="whitespace-nowrap px-4 py-3">
                    <x-badge
                        :label="__('keywords.overdue_days', ['days' => $daysPastDue])"
                        color="red"
                    />
                </td>
                ...
                <td class="whitespace-nowrap px-4 py-3 text-end text-sm">
                    <div class="flex items-center justify-end gap-1">
                        ...
                        @canany(['manage_sales', 'pay_sales'])
                            <button
                                wire:click="openPayModal({{ $sale->id }})"
                        ...
                        @if ($customer)
                            <a
                                href="{{ route('customers.view', $customer) }}"
        ...
        @empty
            <x-empty-state :title="__('keywords.no_overdue_installments')" :colspan="9" />
        @endforelse
    </x-data-table>
    ...
    <x-modal name="pay-sale" title="{{ __('keywords.pay_installment') }}" maxWidth="lg">
        <x-slot:body>
            <div class="space-y-5">
                ...
                <x-input
                    name="payAmount"
                    label="{{ __('keywords.amount') }}"
                    placeholder="{{ __('keywords.enter_amount') }}"
                ...
                <x-select
                    name="payMethod"
                    label="{{ __('keywords.payment_method') }}"
                    :options="['cash' => __('keywords.cash'), 'bank_transfer' => __('keywords.bank_transfer')]"
                    wire:model="payMethod"
                    :placeholder="__('keywords.select_payment_method')"
                />

                <x-textarea
                    name="payNote"
                    label="{{ __('keywords.note') }}"
                    placeholder="{{ __('keywords.enter_note') }}"
                    wire:model="payNote"
                />
                ...
                <label class="inline-flex items-center gap-2 text-sm font-medium text-gray-700 cursor-pointer">
                    <input
                        type="checkbox"
                        wire:model.live="printAfterPayment"
