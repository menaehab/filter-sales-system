    ...
    <div class="border-2 border-black p-6">
        ...
        <div class="grid grid-cols-2 gap-4 mb-6 text-sm">
            <div>
                ...
                <p class="mb-2"><strong>{{ __('keywords.time') }}:</strong>
                    {{ $payment->created_at->format('H:i') }}</p>
            </div>
        ...
        @if ($payment->allocations->count() > 0)
            <div class="mb-6">
                <h3 class="text-sm font-semibold mb-3 border-b border-black pb-2">
                    {{ __('keywords.payment_allocations') }}
                </h3>

                <table class="w-full border-collapse border border-black text-sm">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="border border-black px-3 py-2 text-start">
                                {{ __('keywords.invoice_number') }}
                            </th>
                            <th class="border border-black px-3 py-2 text-end">
                                {{ __('keywords.invoice_total') }}
                            </th>
                            <th class="border border-black px-3 py-2 text-end">
                                {{ __('keywords.paid_amount') }}
                            </th>
                            <th class="border border-black px-3 py-2 text-end">
                                {{ __('keywords.remaining_amount') }}
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            @if ($allocation->sale)
                                <tr>
                                    ...
                                    <td class="border border-black px-3 py-2 text-end">
                                        {{ number_format($allocation->sale->total_price, 2) }}
                                    </td>
                                    <td class="border border-black px-3 py-2 text-end font-semibold text-green-600">
                                        {{ number_format($allocation->amount, 2) }}
                                    </td>
                                    <td
                                        class="border border-black px-3 py-2 text-end {{ $allocation->sale->remaining_amount > 0 ? 'text-red-600 font-semibold' : 'text-gray-500' }}">
        ...
        @if ($payment->note)
            <div class="mb-6 p-3 bg-gray-50 border border-gray-300 rounded-lg">
                <p class="text-sm">
                    <strong>{{ __('keywords.note') }}:</strong> {{ $payment->note }}
                </p>
            </div>
        @endif
            ...
            <div class="border-t-2 border-black pt-4 mb-4">
                <div class="text-center">
                    <p class="text-sm text-gray-600 mb-1">
                        {{ __('keywords.customer_balance') }}
                    </p>
                    <p
                        class="text-2xl font-bold {{ $payment->customer->balance < 0 ? 'text-red-600' : 'text-green-600' }}">
