<div class="max-w-4xl mx-auto p-8 bg-white">
    ...
    <div class="no-print flex items-center justify-between mb-6 border-b pb-4">
        <a href="{{ url()->previous() }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors">
    ...
    <div class="border-2 border-black p-6">
        ...
        <div class="grid grid-cols-2 gap-4 mb-6 text-sm">
            <div>
                ...
                <p class="mb-1"><strong>{{ __('keywords.date') }}:</strong> {{ $sale->created_at->format('Y-m-d') }}</p>
                <p class="mb-1"><strong>{{ __('keywords.time') }}:</strong> {{ $sale->created_at->format('H:i') }}</p>
                @if($sale->dealer_name)
                    <p class="mb-1"><strong>{{ __('keywords.dealer_name') }}:</strong> {{ $sale->dealer_name }}</p>
                @endif
            </div>
            <div>
                <p class="mb-1"><strong>{{ __('keywords.customer') }}:</strong> {{ $sale->customer?->name ?? '—' }}</p>
                @if($sale->customer)
                    @if($sale->customer->phone)
                        <p class="mb-1"><strong>{{ __('keywords.phone') }}:</strong> {{ $sale->customer->phone }}</p>
                    @endif
                    @if($sale->customer->address)
                        <p class="mb-1"><strong>{{ __('keywords.address') }}:</strong> {{ $sale->customer->address }}</p>
                    @endif
            ...
            <tbody>
                @foreach($sale->items as $index => $item)
                    <tr>
                        ...
                        <td class="border border-black px-3 py-2">{{ $item->product?->name ?? __('keywords.product_deleted') }}</td>
                        <td class="border border-black px-3 py-2 text-center">{{ rtrim(rtrim(number_format($item->quantity, 2), '0'), '.') }}</td>
                        <td class="border border-black px-3 py-2 text-start">{{ number_format($item->sell_price, 2) }}</td>
                        <td class="border border-black px-3 py-2 text-start font-semibold">{{ number_format($item->sell_price * $item->quantity, 2) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
            ...
            <div class="flex justify-end">
                <div class="w-64 space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold">{{ number_format($sale->total_price, 2) }} {{ __('keywords.currency') }}</span>
                    </div>
                    <div class="flex justify-between text-green-700">
                        <span>{{ __('keywords.paid_amount') }}:</span>
                        <span class="font-semibold">{{ number_format($sale->paid_amount, 2) }} {{ __('keywords.currency') }}</span>
                    </div>
                    <div class="flex justify-between border-t-2 border-black pt-2 {{ $sale->remaining_amount > 0 ? 'text-red-600' : 'text-green-600' }}">
                        <span class="font-bold">{{ __('keywords.remaining_amount') }}:</span>
                        <span class="font-bold text-lg">{{ number_format($sale->remaining_amount, 2) }} {{ __('keywords.currency') }}</span>
                    </div>
                    @if($sale->isInstallment())
                        <div class="flex justify-between border-t border-gray-300 pt-2 text-blue-600">
                            ...
                            <span class="font-semibold">{{ number_format($sale->installment_amount, 2) }} {{ __('keywords.currency') }}</span>
                        </div>
                        <div class="flex justify-between text-blue-600">
                            <span>{{ __('keywords.installment_months') }}:</span>
                            <span class="font-semibold">{{ $sale->installment_months }} {{ __('keywords.month') }}</span>
                        </div>
                    @endif
                </div>
            </div>
