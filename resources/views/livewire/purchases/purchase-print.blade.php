    ...
    <div class="border-2 border-black p-6">
        ...
        <div class="grid grid-cols-2 gap-4 mb-6 text-sm">
            <div>
                ...
                <p class="mb-1"><strong>{{ __('keywords.date') }}:</strong> {{ $purchase->created_at->format('Y-m-d') }}</p>
                <p class="mb-1"><strong>{{ __('keywords.time') }}:</strong> {{ $purchase->created_at->format('H:i') }}</p>
            </div>
            <div>
                <p class="mb-1"><strong>{{ __('keywords.supplier') }}:</strong> {{ $purchase->supplier?->name ?? $purchase->supplier_name }}</p>
                @if($purchase->supplier)
                    @if($purchase->supplier->phone)
                        <p class="mb-1"><strong>{{ __('keywords.phone') }}:</strong> {{ $purchase->supplier->phone }}</p>
                    @endif
            ...
            <tbody>
                @foreach($purchase->items as $index => $item)
                    <tr>
                        ...
                        <td class="border border-black px-3 py-2">{{ $item->product?->name ?? $item->product_name }}</td>
                        <td class="border border-black px-3 py-2 text-center">{{ rtrim(rtrim(number_format($item->quantity, 2), '0'), '.') }}</td>
                        <td class="border border-black px-3 py-2 text-end">{{ number_format($item->cost_price, 2) }}</td>
                        <td class="border border-black px-3 py-2 text-end font-semibold">{{ number_format($item->cost_price * $item->quantity, 2) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
            ...
            <div class="flex justify-end">
                <div class="w-64 space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold">{{ number_format($purchase->total_price, 2) }} {{ __('keywords.currency') }}</span>
                    </div>
                    <div class="flex justify-between text-green-700">
                        <span>{{ __('keywords.paid_amount') }}:</span>
                        <span class="font-semibold">{{ number_format($purchase->paid_amount, 2) }} {{ __('keywords.currency') }}</span>
                    </div>
                    <div class="flex justify-between border-t-2 border-black pt-2 {{ $purchase->remaining_amount > 0 ? 'text-red-600' : 'text-green-600' }}">
                        <span class="font-bold">{{ __('keywords.remaining_amount') }}:</span>
                        <span class="font-bold text-lg">{{ number_format($purchase->remaining_amount, 2) }} {{ __('keywords.currency') }}</span>
                    </div>
                    @if($purchase->isInstallment())
                        <div class="flex justify-between border-t border-gray-300 pt-2 text-blue-600">
                            ...
                            <span class="font-semibold">{{ number_format($purchase->installment_amount, 2) }} {{ __('keywords.currency') }}</span>
                        </div>
                        <div class="flex justify-between text-blue-600">
                            <span>{{ __('keywords.installment_months') }}:</span>
                            <span class="font-semibold">{{ $purchase->installment_months }} {{ __('keywords.month') }}</span>
                        </div>
                    @endif
                </div>
            </div>
